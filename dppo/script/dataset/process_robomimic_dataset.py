...
# updated code to create a .npz file from a certain mask in the hdf5 file

def make_dataset(load_path, save_dir, save_name_prefix, val_split, normalize, mask=None):
    ...
    with h5py.File(load_path, "r") as f:
        ...
        if not mask:
            ...
            try:
                demo_names = list(f["mask"][mask][()])
                # Convert bytes to strings if necessary
                if len(demo_names) > 0 and isinstance(demo_names[0], bytes):
                    demo_names = [name.decode('utf-8') for name in demo_names]

                demos = [demo for demo in demo_names if demo in f["data"].keys()]
                inds = np.argsort([int(elem[5:]) for elem in demos])
                demos = [demos[i] for i in inds]
            except Exception:
                print(f"Mask {mask} not present in the given HDF5 file")
